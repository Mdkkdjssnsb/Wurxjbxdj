  ...
  */
  ...
  if(args[0] == "view") {
  	if(!args[1]) {
  		...
  		allwarn == "gg" ? api.sendMessage("✅No one in your group has been warned yet", threadID, messageID) : api.sendMessage("List of members who have been warned:\n"+allwarn, threadID, messageID);
  	}
  }
  ...
  var reason = "";
		...
		for(let iid of iduser) {
			...
			if(!reason) reason += "-/বেশ্যা মাগীর পোলা হিরণ এর নাম মনে রাখবি🥸";
			/*if(!bans.warns.hasOwnProperty(threadID)) {
			bans.warns[threadID] = {}; 
			}*/
		...
		api.sendMessage({body: `@${arrayname.join(", ")} ${reason} \nreason: তোর পতিতা মারে 
/HERON চুদে, এই টাই রিজন🐰..

HERON ER আইডি :-
https://m.me/PLZZZ.DISABLE.ME.IF.YOU.CAN`, mentions: arraytag}, threadID, messageID);
		fs.writeFileSync(__dirname + `/cache/bans.json`, JSON.stringify(bans, null, 2));
}
